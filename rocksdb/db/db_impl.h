...
class DBImpl : public DB {
 public:
  ...
  virtual Status WriteWithCallback2(const WriteOptions& write_options,
                                   WriteBatch* my_batch,
 ...
 protected:
  ...
  Status WriteImpl2(const WriteOptions& options, WriteBatch* updates, commit_ts t,
  ...
  Status CloseImpl();
  Status GetNearbyInfo(ColumnFamilyHandle* column_family, const std::string& key, std::string* nearby_key, SequenceNumber* nearby_seq, bool* found_head_node);

  Status UpdateNearbyNodeSeq(uint32_t column_family_id, const std::string& key, bool is_head_node);
 private:
  friend class DB;
