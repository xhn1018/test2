...
bool Getshift(Slice *input,std::string* shift){
   ...
   // if(k == "0000") *shift = 0;
   // else{
    //   ttconvert2(k,shift,0);
   // }
   // std::cout<<"shift 2 "<<*shift<<std::endl;
    input->remove_prefix(4);
    return 0;
}
Status ReadRecordFromWriteBatch2(Slice* input, char* tag,
                                uint32_t* column_family, Slice* key,
                                Slice* value, Slice* blob, Slice* xid, std::string *shift ) {
  ...
  *column_family = 0;  // default
  switch (*tag) {
    ...
    case kTypeValue:
      if (!GetLengthPrefixedSlice(input, key) ||Getshift(input,shift)||
...
void WriteBatch::checkbatch(SequenceNumber t){



//clock_t start,end; 
  //start = clock();
 int iterate1,iterate2;
  std::string shift;
  Slice input(rep_);
  std::string re = rep_;
  ...
  while (!input.empty()) {
      ...
      s = ReadRecordFromWriteBatch2(&input, &tag, &column_family, &key, &value,
                                   &blob, &xid, &shift);
      //sstd::cout<<shift<<std::endl;
      iterate2 =input.size();

    switch (tag) {
      case kTypeValue: 
        //SequenceNumber readseq;
          SequenceNumber readseq;
        if(shift!="0000"){
         //  std::cout<<"write"<<std::endl;
           // std::cout<<"shift :"<<shift<<std::endl;
            ttconvert2(shift,&readseq,1);
            //std::cout<<"t :"<<t<<"readseq :"<<readseq<<std::endl;
          //  std::cout<<"t :"<<t<<"readseq :"<<readseq<<std::endl;
            if(t<=readseq){ 

            
            // std::cout<<"read"<<std::endl;
            //std::cout<< "re size"<< re.size() <<"start "<< re.size()-iterate1 <<"end "<<iterate1-iterate2<<std::endl;
            re.erase(re.size()-iterate1,iterate1-iterate2);
             UNUSED(iterate1);  UNUSED(iterate2);
            }
        }
        break;
      default:
...
//double duration =(double)(end-start)/CLOCKS_PER_SEC;
  //  printf("check batch%f\n",duration); // 4.015
   // return re;
}
std::string check(std::string rep_,commit_ts t){
  int iterate1,iterate2;
  std::string shift;
  Slice input(rep_);
  std::string re = rep_;
  ...
  while (!input.empty()) {
    ...
    switch (tag) {
      case kTypeValue:
        ...
        if(1>0){
        //std::cout<< "re size"<< re.size() <<"start "<< re.size()-iterate1 <<"end "<<iterate1-iterate2<<std::endl;
        re.erase(re.size()-iterate1-1,iterate1-iterate2);
        }
        break;
      default:
        break;
...
//clock_t start,end; 
 ...
 // std::cout<<check(rep_,k)<<std::endl;
  input.remove_prefix(WriteBatchInternal::kHeader);
  Slice key, value, blob, xid;
  // Sometimes a sub-batch starts with a Noop. We want to exclude such Noops as
  // the batch boundary symbols otherwise we would mis-count the number of
  // batches. We do that by checking whether the accumulated batch is empty
  // before seeing the next Noop.ryF
  bool empty_batch = true;
  int found = 0;
  Status s;
  SequenceNumber shift =0; 
  std::string shiftplus;
  char tag = 0;
  uint32_t column_family = 0;  // default
  ...
  while (((s.ok() && !input.empty()) || UNLIKELY(s.IsTryAgain())) &&
     ...
     // std::cout<<"again1  ::"<< (s.IsTryAgain())<<std::endl;
      assert(!s.IsTryAgain());
    if (LIKELY(!s.IsTryAgain())) {//std::cout<<"again2"<<std::endl;
    ...
    switch (tag) {
      ...
      case kTypeValue:
        ...
        if(shiftplus == "0000") {shift = 0;}
       else{
       //std::cout<<"size: "<<shiftplus.size()<<std::endl;
       ttconvert2(shiftplus,&shift,0);
     //std::cout<<"Put :"<< key.ToString()<<"  value :" <<value.ToString()  <<std::endl;
      }
  ...
  if (!s.ok()) {
    std::cout<<"writebatch read error"<<std::endl;
    assert(0);
    return s;
  }
  if (found != WriteBatchInternal::Count(this)) {
   // std::cout<<"count problem"<<std::endl;
        return Status::OK();
...
Status WriteBatchInternal::Put2(WriteBatch* b, uint32_t column_family_id,
                               const Slice& key, const Slice& value,std::string k) {
  UNUSED(k);
  if (key.size() > size_t{port::kMaxUint32}) {
    return Status::InvalidArgument("key is too large");
  }
  ...
  PutLengthPrefixedSlice(&b->rep_, key);
  b->rep_.append(k);
  //std::cout<<"asddddddddddddddddddddddddddddddddd"<<std::endl;
  //b->k();
  PutLengthPrefixedSlice(&b->rep_, value);
  //b->k();
  b->content_flags_.store(
...
Status WriteBatch::Put2(ColumnFamilyHandle* column_family, const Slice& key,
                       const Slice& value,std::string k) {
  return WriteBatchInternal::Put2(this, GetColumnFamilyID(column_family), key,
                                 value,k);
}
...
class MemTableInserter : public WriteBatch::Handler {
 ...
 public:
  ...
  Status PutCFImpl1(uint32_t column_family_id, const Slice& key,
    ...
    //std::cout<<"begin sequence_"<<sequence_<<std::endl; 
    Status seek_status;
    ...
    seq = sequence_;
    Status ret_status;
   // std::cout<<"seq :"<<sequence_<<std::endl;
    if(shift1 != 0)  {
      //  std::cout<<"read shift"<<std::endl;
      
        sequence_ = ((sequence_>>24)<<24) + (shift1<<12)+((sequence_>>12)%4096-shift1); 
    }
   //std::cout<<"putcfseq "<<sequence_  <<std::endl;
    ...
    if (!moptions->inplace_update_support) {
      bool mem_res =
        ...
        //std::cout<<"insert key :"<< key.ToString()<<"  value :" <<value.ToString()  <<"   sequence_ :"<<sequence_<<std::endl;
      if (UNLIKELY(!mem_res)) {
      ...
      if (mem->UpdateCallback(sequence_, key, value)) {
        ...
        if (status == UpdateStatus::UPDATED_INPLACE) {
          ...
          //std::cout<<"insert key :"<<"  value :"<<"sequence_ :"<<sequence_<<std::endl;
          assert(mem_res);
    ...
    MaybeAdvanceSeq();
    CheckMemtableFull();
  // std::cout<<"End sequence_"<<sequence_<<std::endl;
    sequence_ = seq  ;
    return ret_status;
  }
...
Status WriteBatchInternal::InsertInto(
  ...
  MemTableInserter inserter(
   ...
   // std::cout<<"insertinto 1"<<std::endl;
  for (auto w : write_group) {
    if (w->CallbackFailed()) {
      continue;
    }
   // std::cout<<"insertinto seq :"<<sequence<<std::endl;
...
Status WriteBatchInternal::InsertInto2(
    WriteThread::WriteGroup& write_group, commit_ts t,SequenceNumber sequence,
    ColumnFamilyMemTables* memtables, FlushScheduler* flush_scheduler,
    bool ignore_missing_column_families, uint64_t recovery_log_number, DB* db,
    bool concurrent_memtable_writes, bool seq_per_batch, bool batch_per_txn) {
    ...
    //std::cout<<"   insert into 2" <<std::endl;
  MemTableInserter inserter(
      sequence, memtables, flush_scheduler, ignore_missing_column_families,
      recovery_log_number, db, concurrent_memtable_writes,
      nullptr /*has_valid_writes*/, seq_per_batch, batch_per_txn);
  for (auto w : write_group) {
    ...
    // std::cout<<"insertinto2 seq :"<<sequence<<std::endl;
    // std::cout<<"insertinto2 seq per :"<<seq_per_batch<<std::endl;
    UNUSED(t);
    w->sequence = inserter.sequence();
    if (!w->ShouldWriteToMemtable()) {
      // In seq_per_batch_ mode this advances the seq by one.
   ...
   // std::cout<<"writebatch read error"<<std::endl;
    ...
    if (!w->status.ok()) {
    //  std::cout<<"w.status lose"<<std::endl;
      return w->status;
    }
    assert(!seq_per_batch || w->batch_cnt != 0);
    assert(!seq_per_batch || inserter.sequence() - w->sequence == w->batch_cnt);
...
Status WriteBatchInternal::InsertInto3(
    WriteThread::Writer* writer, commit_ts t,SequenceNumber sequence,
    ColumnFamilyMemTables* memtables, FlushScheduler* flush_scheduler,
    bool ignore_missing_column_families, uint64_t log_number, DB* db,
    bool concurrent_memtable_writes, bool seq_per_batch, size_t batch_cnt,
    bool batch_per_txn) {
