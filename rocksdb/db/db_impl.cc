...
Status DBImpl::GetNearbyInfo(ColumnFamilyHandle *column_family, const std::string &key,
 ...
 // std::cout<<"head :"<<found_head_node<<std::endl;
  ReturnAndCleanupSuperVersion(cfd, sv);

  return s;
}

Status DBImpl::UpdateNearbyNodeSeq(uint32_t column_family_id, const std::string& key,
                                   bool is_head_node) {
  Status s;
