...
port::RWMutex* MemTable::GetLock(const Slice& key) {
  ...
  //std::cout<<"read lock size"<<locks_.size()<<std::endl;
...
bool MemTable::Add(SequenceNumber s, ValueType type,
  ...
  if (!allow_concurrent) {
    ...
    //std::cout<<"first_seqno :"<<first_seqno_<<" /  s : "<<s <<std::endl;
...
static bool SaveNearbyValue(void* arg, const char* entry) {
  ...
  //std::cout<<"key lenth "<< key_length <<std::endl;
  Slice user_key(key_ptr, key_length - 8);
 // std::string m = user_key.ToString();
  //std::cout<<"user key  ::"<<m<<std::endl;
