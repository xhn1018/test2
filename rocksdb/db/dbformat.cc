...
std::string ttconvert1(SequenceNumber s){
    ...
    std::string k ="";
    k = k +(static_cast<char>(a/64));
    k = k +(static_cast<char>(a%64));
    k = k +(static_cast<char>(b/64));
    k = k +(static_cast<char>(b%64));

   // std::cout<<"string k: "<<k<<" "<<k.at(3) << std::endl;
    return k;
}

void  ttconvert2(std::string k,SequenceNumber *s1,bool a){  
    if(a==0)
    *s1 = 64*(int)k.at(0)+(int)k.at(1) ;
    else
    *s1 = 64*(int)k.at(2)+(int)k.at(3) ;



  //std::cout<< " three "<<(int)k.at(3)<<std::endl;
}
commit_ts Getshift(uint64_t seq){
    commit_ts t;
    t.s = seq%4096;
    t.c =0;
    return t;
}
std::string shift_to(commit_ts t){
   int k=0 ;
   if(t.c){
     k = t.s ;
   }
   ...
   std::string shift;
   shift =std:: to_string(k);
   return shift;
}
commit_ts to_shift(std::string t){
   commit_ts commit_t;
   int a = std::stoi(t);
   if(a>4096){
     a = a-4096;
...
bool compare(commit_ts a, commit_ts b)
{
    if(a.c==b.c){
    return (a.s>b.s);
    }
