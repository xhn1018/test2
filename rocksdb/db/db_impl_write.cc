...
Status DBImpl::WriteWithCallback2(const WriteOptions& write_options,
                                 WriteBatch* my_batch,
                                 WriteCallback* callback,commit_ts t) {
  UNUSED(t);
  Status s;
  s = WriteImpl2(write_options, my_batch,t, callback, nullptr);
  //std::cout<<"callback status"<<s.getState()<<std::endl;
  return s;
  //return WriteImpl2(write_options, my_batch,t, callback, nullptr);
}
#endif  // ROCKSDB_LITE
...
Status DBImpl::WriteImpl2(const WriteOptions& write_options, 
                         WriteBatch* my_batch, commit_ts t,WriteCallback* callback,
                         uint64_t* log_used, uint64_t log_ref,
                         bool disable_memtable, uint64_t* seq_used,
                         size_t batch_cnt,
                         PreReleaseCallback* pre_release_callback) {
...
//std::cout<<"disable"<<std::endl;
  ...
  if (w.state == WriteThread::STATE_PARALLEL_MEMTABLE_WRITER) {
    ...
    if (write_thread_.CompleteParallelMemTableWriter(&w)) {
      // we're responsible for exit batch groupwa
      for (auto* writer : *(w.write_group)) {
        if (!writer->CallbackFailed() && writer->pre_release_callback) {
          assert(writer->sequence != kMaxSequenceNumber);
          Status ws = writer->pre_release_callback->Callback(writer->sequence,
 ...
 //std::cout<<my_batch->TictocSequence<<std::endl;
      ...
      if (status.ok() && !write_options.disableWAL) {
        ...
        // LastAllocatedSequence is increased inside WriteToWAL under
        // wal_write_mutex_ to ensure ordered events in WAL
        std::cout<<"concurent write :"<< last_sequence<< std::endl;
        status = ConcurrentWriteToWAL(write_group, log_used, &last_sequence,
                                      seq_inc);

      } else {
        // Otherwise we inc seq number for memtable writes
         std::cout<<"concurent write2 :"<<last_sequence<<std::endl;
        last_sequence = versions_->FetchAddLastAllocatedSequence(seq_inc);
      }
    }
    assert(last_sequence != kMaxSequenceNumber);
    const SequenceNumber current_sequence = last_sequence + 1;
    last_sequence += 1;
   // std::cout<<"last_sequence :"<<t.s<<std::endl;
   // std::cout<<"commit seq :"<<t.s<<std::endl;
   // std::cout<<"current_sequence :"<<current_sequence<<std::endl;
   //  std::cout<<"seq_inc; :"<<seq_inc<<std::endl;
    if (status.ok()) {
      ...
      if (!parallel) {
        // w.sequence will be set inside InsertInto
        w.status = WriteBatchInternal::InsertInto2(
            write_group, t,current_sequence, column_family_memtables_.get(),
            &flush_scheduler_, write_options.ignore_missing_column_families,
            0 /*recovery_log_number*/, this, parallel, seq_per_batch_,
            batch_per_txn_);
        ...
        write_group.last_sequence = kMaxSequenceNumber;
        write_group.running.store(static_cast<uint32_t>(write_group.size),
                                  std::memory_order_relaxed);
        write_thread_.LaunchParallelMemTableWriters(&write_group);
        in_parallel_group = true;
        ...
        if (w.ShouldWriteToMemtable()) {std::cout<<"should write to memtbale"<<std::endl;
          ColumnFamilyMemTablesImpl column_family_memtables(
              ...
              versions_->GetColumnFamilySet());
          assert(w.sequence == current_sequence);
          //std::cout<<"insert into 1"<<std::endl;
          w.status = WriteBatchInternal::InsertInto(
              &w, w.sequence, &column_family_memtables, &flush_scheduler_,
              write_options.ignore_missing_column_families, 0 /*log_number*/,
              this, true /*concurrent_memtable_writes*/, seq_per_batch_,
  ...
  if (need_log_sync) {
    ...
    MarkLogsSynced(logfile_number_, need_log_dir_sync, status);
    mutex_.Unlock();
    // Requesting sync with two_write_queues_ is expected to be very ke. We
    // hence provide a simple implementation that is not necessarily efficient.
    if (two_write_queues_) {
      if (manual_wal_flush_) {
        status = FlushWAL(true);
  ...
  if (should_exit_batch_group) {
   ...
   // if (!w.status.ok())   std::cout<<"last final status propblem"<<std::endl;
    MemTableInsertStatusCheck(w.status);
    write_thread_.ExitAsBatchGroupLeader(write_group, status);
  }
 //if (!status.ok())   std::cout<<"final status propblem1"<<std::endl;
  if (status.ok()) {
    
     status = w.FinalStatus();
 //   if (!status.ok())   std::cout<<"final status propblem"<<std::endl;
  }
  return status;
}
...
Status DBImpl::PreprocessWrite(const WriteOptions& write_options,
  ...
  //std::cout<<"pre status"<<status.ok()<<std::endl;
  return status;
}
