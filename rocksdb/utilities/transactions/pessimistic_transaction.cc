...
//  (found in the LICENSE.Apache file ifchen the root directory).
  ...
  DBImpl* db_impl = static_cast_with_check<DBImpl, DB>(db_->GetRootDB());
   ...
   for (auto& key_map_iter : GetTrackedKeys()) { 
    ...
    for (const auto& key_iter1 : keys) {  
     ...
     // std::cout<<"wts "<< w_ts<<"rts "<<r_ts<<std::endl;
     }
    }
    ...
    SequenceNumber w_ts ;
    SequenceNumber r_ts ;
    UnPackSequence(&w_ts,&r_ts, commit_t.s);
   // std::cout<<"new: "<<"wts "<< w_ts<<"rts "<<r_ts<<std::endl;
    UNUSED(commit_t);
    //std::cout<<"cts"<<commit_t.s<<std::endl;
   // GetWriteBatch()->GetWriteBatch()->TictocSequence =ttseq<<12;
    //GetWriteBatch()->GetWriteBatch()->k();
    //std::cout<<commit_t.s<<std::endl;
//std::cout<<"xgn"<<std::endl;
    
  s = db_impl->WriteWithCallback2(
      write_options_,GetWriteBatch()->GetWriteBatch() , &callback,commit_t);
  return s;
}
     ...
     // auto cfd = cfh->cfd();
      SuperVersion* sv = dbimpl_->GetAndRefSuperVersion(cfh_id);
       ...
       if(found_record_for_key == 0) {
            //std::cout<<"not found 1  :"<<key_str <<std::endl;
           seq =0x1<<12; 
       }
...
Status PessimisticTransaction::TryLock2(ColumnFamilyHandle* column_family,
                                              const Slice& key, bool read_only,
                                              bool exclusive, SequenceNumber* returnseq, bool untracked) {
    if (untracked) {
      return Status::OK();
    }
    uint32_t cfh_id = GetColumnFamilyID(column_family);
...
Status PessimisticTransaction::CheckTransactionForConflicts(DB* db) {
    ...
    // Since we are on the write thread and do not want to block other writers,
    // we will do a cache-only conflict s.  This can result in TryAgain
    // getting returned if there is not sufficient memtable history to check
    // for conflicts.
    for (auto& key_map_iter : GetTrackedKeys()) { 
    const auto& keys = key_map_iter.second;
    ...
    //int k=ttseq;
    //printf("%d",k);
    //std::cout<<"ttseq"<<ttseq<<std::endl;

    //std::cout<<"xhn"<<std::endl;
    
    return TransactionUtil::CheckKeysForConflicts1(db_impl, GetTrackedKeys(),
  ...
  for (auto& key_map_iter : key_map) {
    ...
    for (const auto& key_iter : keys) {
      const auto& key = key_iter.first;
       ...
       if(key_iter.second.exclusive== 1)
      {
