...
Status s1 = TryLock2(column_family, key, true /* read_only */, false/* exclusive */,&a);
//  Status s1 = TryLock(column_family, key, false /* read_only */, true /* exclusive */);
 //  std::cout<<"get num:"<<a<<std::endl;  
  std::string shift;
  shift = ttconvert1(a%(4096*4096));
 //shift = "0000";
  //std::cout<<"put shift :"<<shift<<std::endl;
  if (s1.ok()) {
   s1 = GetBatchForWrite()->Put2(column_family, key,*value,shift);
  // if (s1.ok()) {
  //    num_puts_++;
  //  }
  }
...
//    s = DoPessimisticLock(column_family, key, false /* read_only */, true /* exclusive */, true /* fail_fast */);
 ...
 std::cout<<"near by :"<<nearby_key<<std::endl;
    // if nearby node exist. add the nearby node to read set
    if (s.ok()) {
    //  uint32_t cfh_id = GetColumnFamilyID(column_family);
     s =
      TryLock(column_family, nearby_key, false /* read_only */, false /* exclusive */);
