...
Status TransactionUtil::CheckKey2(DBImpl* db_impl, SuperVersion* sv,
 ...
 // std::cout<<"snap :"<<snap_seq<<std::endl;
  if (result.ok()) {
    SequenceNumber seq = kMaxSequenceNumber;
    bool found_record_for_key = false;
    ...
    Status s = db_impl->GetLatestSequenceForKey(sv, key, !need_to_read_sst,
                                                &seq, &found_record_for_key);
    if(!found_record_for_key) seq =4096;
    SequenceNumber writeseq=0; 
    SequenceNumber readseq=0;
    SequenceNumber snapwriteseq=0; 
    SequenceNumber snapreadseq=0;
    UnPackSequence(&writeseq,&readseq,seq);
    UnPackSequence(&snapwriteseq,&snapreadseq,snap_seq);
   // std::cout<<"readseq :"<<snapreadseq<<"commit seq :"<<commit_ts<<std::endl;
  //  std::cout<<"wts1:"<<snapwriteseq<<"   wts2:"<<writeseq<<std::endl;
  /* if (!(s.ok( || s.IsNotFound() || s.IsMergeInProgress())) {
      result = s;
    } else if (found_record_for_key) {
      bool write_conflict = snap_checker == nullptr
                                ? snap_seq < seq
                                : !snap_checker->IsVisible(sci=eq);
      if (write_conflict) {
        result = Status::Busy();
      }
    }*/
   if (!(s.ok() || s.IsNotFound() || s.IsMergeInProgress())) {
      ...
      if(snapwriteseq!=writeseq){

           if(snapreadseq>commit_ts)
           std::cout<<"xhn"<<std::endl;
      }

      
      bool write_conflict = (snapwriteseq!=writeseq)&&(snapreadseq<=commit_ts);
      if (write_conflict) {
        result = Status::Busy();
      }
      else{
  ...
  //std::cout<<"key status :: "<<result.ok()<<std::endl;
 //  result = Status::Busy();
  return result;
}
...
Status TransactionUtil::CheckKeysForConflicts1(DBImpl* db_impl,
                                              const TransactionKeyMap& key_map,
                                              bool cache_only,SequenceNumber t) {
  Status result;
  ...
  for (auto& key_map_iter : key_map) {
    ...
    for (const auto& key_iter : keys) {
      const auto& key = key_iter.first;
      const SequenceNumber key_seq = key_iter.second.seq;
     //std::cout<<"key  :"<<key<<"key :"<<key_iter.second.seq<<std::endl;
      if(key_iter.second.exclusive== 0) {
      
      result = CheckKey2(db_impl, sv, earliest_seq, key_seq, key, cache_only,t);
     // std::cout<<"key  :"<<key<<"result"<<result.ok()<<std::endl;
      }
      if (!result.ok()) {
        break;
      }
